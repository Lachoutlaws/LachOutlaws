// home.js — behavior for theme, header+menu, section linking, overlay gallery, mobile rail, etc.
document.addEventListener("DOMContentLoaded", () => {
  ...
  const slideFromAnchor = (a, inferMetaFn) => {
    ...
    const overview = a.dataset.overview || "";               // carries overview text
  ...
  // Expose hooks so appContent.js can trigger a rebuild after JSON injection
  ...
  // Update static label inside center card
  function updateGalleryTopLabelText(slideMeta) {
    const gal = document.getElementById("gallery");
    if (!gal || !slideMeta) return;

    const label = gal.querySelector(".gallerySection.is-center .galleryTopLabel");
    if (!label) return;

    const nice = (slideMeta.sectionLabel || slideMeta.section || "").trim();
  ...
  const gal = $("#gallery");
  if (!gal) {
    // If gallery isn't on this page, there's nothing else to bind.
    return;
  }
  ...
  const btnPrev = gal.querySelectorAll(".arrow-left, .gallerySectionArrow.is-prev");
  const btnNext = gal.querySelectorAll(".arrow-right, .gallerySectionArrow.is-next");
  ...
  const overviewEl = gal.querySelector(".overview, .projectCardOverview, .projectCardInfo"); // overview node
  ...
  const setCaseContent = (s) => {
    ...
    // ✅ update the static label in the card
    updateGalleryTopLabelText(s);
  ...
  const onDomChanged = () => {
    ...
    // label text will be updated next time render/setCaseContent runs
  };
