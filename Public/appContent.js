...
async function init() {
  // Only run where needed: home sections OR a case-study rail is present
  const needsSections = SECTIONS.some(s => document.querySelector(s.toc) || document.querySelector(s.tiles));
  const hasRail       = !!document.querySelector(".rail");
  if (!needsSections && !hasRail) return;

  try {
    // Cache-bust to avoid stale content.json in aggressive caches
    ...
    // (1) Render home sections (if present on this page)
    ...
    // Let the home overlay rescan after injection (if those hooks exist)
    ...
    // (2) Hydrate PREVIOUS/NEXT rail on case-study pages
...
function tileHTML(item) {
  ...
  // Anchor if link exists; otherwise a div (still openable into the overlay)
  const startTag = hasLink
